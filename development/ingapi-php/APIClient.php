...
class APIClient {
  ...
  protected function sanitizeForSerialization($data)
  {
    if (is_scalar($data) || null === $data) {
      ...
      $values = array();
      foreach ($data::$swaggerNames as $property => $internalName) {
        $values[$property] = $this->sanitizeForSerialization($data->$internalName);
      }
      $sanitized = $values;
    } else {
  ...
  public static function deserialize($data, $class)
  {
    if (null === $data) {
      ...
      // XML to JSON conversion means single element not always array
      // Convert this object into a single element array
      if (!is_array($data)) {
      ...
      $instance = new $class();
      foreach ($instance::$swaggerNames as $property => $internalName) {
        if (isset($data->$property)) {
          $type = $instance::$swaggerTypes[$internalName];
          $instance->$internalName = self::deserialize($data->$property, $type);
        }
      }
